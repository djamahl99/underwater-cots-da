...
class MultiDilationResnetBlock_attention(nn.Module):
    ...
    def forward(self, x_hdr, x_relight):
        shape = (288, 512)
        print("A: x hdr", x_hdr.shape, "x_relight", x_relight.shape, x_relight.shape[-2:])
        x_hdr = nn.functional.upsample(x_hdr, size=shape)
        x_relight = nn.functional.upsample(x_relight, size=shape)
        print("B: x hdr", x_hdr.shape, "x_relight", x_relight.shape)
